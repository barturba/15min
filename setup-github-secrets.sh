...
echo "   • KAMAL_HOST: Server IP address (5.78.133.135)"
...
gh secret set KAMAL_HOST --body "5.78.133.135"
...
echo ""
echo -e "${YELLOW}Run this on your server (5.78.133.135):${NC}"
...
if [[ $TEST_SSH =~ ^[Yy]$ ]]; then
    ...
    if ssh -i "$SSH_KEY_FILE" -o StrictHostKeyChecking=no -o ConnectTimeout=10 root@5.78.133.135 "echo 'SSH connection successful'"; then
